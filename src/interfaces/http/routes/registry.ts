...
export async function registerRegistryRoutes(app: FastifyInstance) {
	app.post('/v1/admin/brokers', async (req, reply) => {
	...
	app.get('/v1/admin/brokers', async () => {
		const pool = getPool();
		const res = await pool.query('SELECT id, code, name FROM registry.brokers ORDER BY id ASC');
		return { items: res.rows };
	});

	app.post('/v1/admin/instruments', async (req, reply) => {
	...
	app.get('/v1/admin/instruments', async () => {
		const pool = getPool();
	...
	app.delete('/v1/admin/watch/broker/:broker_id', async (req, reply) => {
	...
	app.delete('/v1/admin/watch/instrument/:instrument_id', async (req, reply) => {
		const schema = z.object({ instrument_id: z.coerce.number().int().positive() });
		const parsed = schema.safeParse(req.params);
		if (!parsed.success)
			return reply.code(400).send({ code: 'BAD_REQUEST', message: 'Invalid instrument_id' });
	...
	app.delete('/v1/admin/watch/:broker_id/:instrument_id', async (req, reply) => {
