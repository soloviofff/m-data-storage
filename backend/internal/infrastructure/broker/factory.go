...
func (f *Factory) createCryptoBroker(config interfaces.BrokerConfig) (interfaces.Broker, error) {
	...
	// Пока создаем базовый брокер
	// В будущем здесь будет создание специализированных криптоброкеров
	broker := NewBaseBroker(config, f.logger)
...
func (f *Factory) createStockBroker(config interfaces.BrokerConfig) (interfaces.Broker, error) {
	...
	// Пока создаем базовый брокер
	// В будущем здесь будет создание специализированных фондовых брокеров
	broker := NewBaseBroker(config, f.logger)
