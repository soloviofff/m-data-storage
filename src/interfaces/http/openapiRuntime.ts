...
import type { OpenAPIObject, PathItemObject, PathsObject } from 'openapi3-ts/oas30';
import { z } from 'zod';
import {
	ReadResponseSchema,
	IngestBodySchema,
...
export function buildOpenApiDocument(serverUrl: string) {
	...
	registry.register(
		'IngestResponse',
		IngestResponseSchema ??
			z.object({ inserted: z.number(), updated: z.number(), skipped: z.number() }),
	);
	registry.register(
		'BrokersUpsertBody',
		BrokersUpsertBodySchema ??
	...
	registry.register(
		...
		RemovedCountResponseSchema ?? z.object({ removed: z.number().int().nonnegative() }),
	);
	registry.register(
		'TasksListResponse',
		TasksListResponseSchema ?? z.object({ items: z.array(z.any()) }),
	);
	registry.register(
		'TaskCompleteBody',
		TaskCompleteBodySchema ?? z.object({ status: z.enum(['done', 'failed']) }),
	);
	registry.register(
		'TaskCompleteResponse',
		TaskCompleteResponseSchema ?? z.object({ item: z.any() }),
	);
	registry.register(
		'ErrorResponse',
		ErrorResponseSchema ?? z.object({ code: z.string(), message: z.string() }),
	);
	...
	const definePath = (path: string, item: PathItemObject): void => {
		(document.paths as PathsObject)[path] = item;
	};
	...
	definePath('/v1/ohlcv', {
		get: {
			...
			responses: {
				'200': {
					description: 'OK',
					content: { 'application/json': { schema: ref('ReadResponse') } },
				},
				'400': {
	...
	definePath('/v1/ingest/ohlcv', {
		post: {
			summary: 'Ingest OHLCV batch',
			requestBody: {
				required: true,
				content: { 'application/json': { schema: ref('IngestBody') } },
			},
			responses: {
				'200': {
					description: 'OK',
					content: { 'application/json': { schema: ref('IngestResponse') } },
				},
				'400': {
					description: 'Bad Request',
					content: { 'application/json': { schema: ref('ErrorResponse') } },
	...
	definePath('/v1/tasks/next', {
		get: {
			...
			responses: {
				'200': {
					description: 'OK',
					content: { 'application/json': { schema: ref('TasksListResponse') } },
				},
				'400': {
					description: 'Bad Request',
					content: { 'application/json': { schema: ref('ErrorResponse') } },
	...
	definePath('/v1/tasks/{id}/complete', {
		post: {
			...
			requestBody: {
				required: true,
				content: { 'application/json': { schema: ref('TaskCompleteBody') } },
			},
			responses: {
				'200': {
					description: 'OK',
					content: { 'application/json': { schema: ref('TaskCompleteResponse') } },
				},
				'400': {
					description: 'Bad Request',
					content: { 'application/json': { schema: ref('ErrorResponse') } },
	...
	definePath('/v1/admin/brokers', {
		get: {
			summary: 'List brokers',
			responses: {
				'200': {
					description: 'OK',
					content: { 'application/json': { schema: ref('BrokersListResponse') } },
				},
			},
		},
		post: {
			...
			requestBody: {
				required: true,
				content: { 'application/json': { schema: ref('BrokersUpsertBody') } },
			},
			responses: {
				'200': {
					description: 'OK',
	...
	definePath('/v1/admin/instruments', {
		get: {
			summary: 'List instruments',
			responses: {
				'200': {
					description: 'OK',
					content: { 'application/json': { schema: ref('InstrumentsListResponse') } },
				},
			},
		},
		post: {
			...
			requestBody: {
				required: true,
				content: { 'application/json': { schema: ref('InstrumentsUpsertBody') } },
			},
			responses: {
				'200': {
					description: 'OK',
	...
	definePath('/v1/admin/watch/broker/{broker_id}', {
		delete: {
			...
			responses: {
				'200': {
					description: 'OK',
					content: { 'application/json': { schema: ref('RemovedCountResponse') } },
				},
			},
		},
	});
